...
class UrlWhitelistCrud:
    ...
    async def get_all_active(self, user_id: Optional[int] = None) -> List[UrlWhitelistEntity]:
        """获取所有启用的白名单规则。
        ...
        async with self._db_manager.get_session() as session:
            if user_id is not None:
                ...
                query = select(UrlWhitelistEntity).where(
                    ...
                    ((UrlWhitelistEntity.user_id == None) | (UrlWhitelistEntity.user_id == user_id))
                )
            else:
                # 获取所有启用的规则
                query = select(UrlWhitelistEntity).where(
    ...
    async def get_by_user_id(self, user_id: int) -> List[UrlWhitelistEntity]:
        """获取用户的所有白名单规则。
        ...
        async with self._db_manager.get_session() as session:
            query = select(UrlWhitelistEntity).where(
                UrlWhitelistEntity.user_id == user_id
            ).order_by(
