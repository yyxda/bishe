 ...
 *
 * 提供发件人规则的添加、编辑、删除功能。
 * 支持多种匹配规则（EMAIL、DOMAIN、DOMAIN-SUFFIX、DOMAIN-KEYWORD）。
 */
...
const RULE_TYPES = [
  { value: 'EMAIL', label: 'EMAIL', desc: '精确匹配邮箱地址' },
  { value: 'DOMAIN', label: 'DOMAIN', desc: '精确匹配邮箱域名' },
  { value: 'DOMAIN-SUFFIX', label: 'DOMAIN-SUFFIX', desc: '匹配邮箱域名后缀' },
  { value: 'DOMAIN-KEYWORD', label: 'DOMAIN-KEYWORD', desc: '匹配邮箱域名关键词' },
]

/**
 * 发件人白名单管理组件。
 *
 * @returns {JSX.Element} 发件人白名单管理界面
 */
export default function SenderWhitelist() {
  ...
  const loadRules = useCallback(async () => {
    setLoading(true)
    try {
      const response = await adminService.getSenderWhitelistRules()
   ...
   */
  const handleOpenAdd = () => {
    setFormData({ ruleType: 'EMAIL', ruleValue: '', description: '' })
    setEditingRule(null)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRule) {
        await adminService.updateSenderWhitelistRule(editingRule.id, {
        ...
        await adminService.createSenderWhitelistRule(
          formData.ruleType,
          formData.ruleValue,
  ...
  const handleDeleteRule = async (rule) => {
    ...
    try {
      await adminService.deleteSenderWhitelistRule(rule.id)
      Toast.success('规则已删除')
      loadRules()
    } catch (error) {
    ...
    <div className="admin-card">
      ...
      <div style={{ marginBottom: '16px', padding: '12px 16px', background: 'var(--color-primary-lighter)', borderRadius: '10px', fontSize: '0.9rem' }}>
        ...
        <ul style={{ margin: '8px 0 0 20px', padding: 0 }}>
          <li><code>EMAIL</code>：精确匹配邮箱地址，如 <code>admin@example.com</code></li>
          <li><code>DOMAIN</code>：精确匹配邮箱域名，如 <code>example.com</code></li>
          <li><code>DOMAIN-SUFFIX</code>：后缀匹配，如 <code>edu.cn</code> 可匹配 <code>xxx@xxx.edu.cn</code></li>
          <li><code>DOMAIN-KEYWORD</code>：关键词匹配，如 <code>university</code> 可匹配所有包含该词的域名</li>
        </ul>
        <p style={{ margin: '8px 0 0', color: 'var(--color-ink-secondary)' }}>
          注意：来自白名单发件人的邮件将直接判定为正常邮件，不进行任何检测。
        </p>
      </div>
